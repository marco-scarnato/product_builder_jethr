...
export const SalaryHeroCards: React.FC<SalaryHeroCardsProps> = ({
  ...
  return (
    <div className={cn('grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4', className)}>
      {/* Card 1 (Sinistra): Netto al Mese (Grande) + Anno (Piccolo) in Verde */}
      ...
      {/* Card 2 (Destra): Totale Trattenute al Mese (Grande) + Anno (Piccolo) in Rosso */}
      <div className="bg-rose-50 border border-rose-200/80 rounded-xl p-5 sm:p-6 space-y-3">
        <div className="flex items-center justify-between">
        ...
        <div>
          ...
          <p className="mt-1.5 text-xs text-rose-800/90 font-medium">
            Trattenute stimate all'anno:{' '}
            <strong className="font-bold text-rose-950">{formatCurrency(totalTaxes)}</strong>
          </p>
        </div>
      </div>
    </div>
